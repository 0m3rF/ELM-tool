...
- **Global scope**: `C:/Projeler/ELM-tool/.augment/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Projeler/ELM-tool/.augment/notes/`)
...
2. Glob `C:/Projeler/ELM-tool/.augment/notes/*.md` (if directory exists) — global notes
...
Global (C:/Projeler/ELM-tool/.augment/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Projeler/ELM-tool/.augment/notes/` — works in any directory
