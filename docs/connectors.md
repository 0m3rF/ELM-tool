...
- Unconstrained NUMERIC, precision above 38, scale greater than precision, UUID, JSON, arrays,
  enums/domains, intervals, and oversized-LOB spill fail preflight; no text or row-by-row fallback
  is selected.
...
batch sequence; MySQL source queries restart from zero and do not support resume keys yet.
...
Receipts remain until job deletion. Full fault-injection, privilege preflight, backup cleanup
recovery, memory-budget accounting for encoded buffers, and performance gates remain outstanding.
...
are checked before staging. Receipt and staging cleanup refuse unrecognized job markers.
...
Other types require explicit conversions. Writes are restricted to the connected user's schema.
