...
- [ ] Run Windows/macOS/Linux native database-driver and OS-keychain acceptance, including Oracle and ODBC setup failures.
  Partial pass on 2026-09-17, Windows PostgreSQL and MySQL only (no macOS/Linux hardware and no
  Oracle Instant Client/SQL Server Driver 18 installed on this Windows host; PostgreSQL's and
  MySQL's drivers are both pure-Rust and need no native client library, unlike Oracle/SQL Server).
  Everything up to now had only run against Linux Docker containers (see §1); this is the first
  time any of it ran as a native Windows process.
  ...
  - The full ignored `crates/elm-connectors/tests/mysql.rs` suite (12 tests) ran natively on
    Windows against a dedicated `mysql:8.4 --local-infile=1` container (`elm_test` database,
    matching the suite's hardcoded default), run single-threaded per the suite's own documented
    requirement (one test temporarily changes the server-wide `LOCAL` policy). All 12 passed,
    covering both the `LOAD DATA LOCAL` path and the prepared-statement fallback when `LOCAL` is
    disabled, on the native Windows `mysql_async` client.
  All fixtures torn down afterward (`docker compose ... down --volumes`, `docker stop`); the
  disposable password file was deleted from the scratch directory. **Not done**: SQL Server and
